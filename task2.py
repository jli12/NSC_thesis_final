...
rules_dict = \
    {'all' : ['fdgo', 'reactgo','delaygo', 'fdanti', 'reactanti', 'delayanti',
              ...
              'dmsgo', 'dmsnogo', 'reactoffgo', 'reactoffanti'],
...
class Trial(object):
    ...
    def add(self, loc_type, locs=None, ons=None, offs=None, strengths=1, mods=None):
        ...
        for i in range(self.batch_size):
            ...
            if loc_type == 'fix_in':
                if ('mode' in self.config and self.config['mode']=='inference') and ('probe' in self.config) and self.config['probe']=='probe':
                    # print(self.config['fxtn_val'])
                    if self.config['fxtn_val']==1: 
                        self.x[:, i, 0] = self.config['fxtn_val'] # want to probe network
                        # print("jeff")

                    elif self.config['fxtn_val']==0.5:
                        l = len(self.x)
                        self.x[:int(l/2), i, 0] = 1 # want to probe network
                        self.x[int(l/2):, i, 0] = 0 # want to probe network
                        # print("jeff2")
                else:
...
def delaygo_(config, mode, anti_response, **kwargs):
    ...
    # if 'tdim' in config: tdim = config['tdim']
    if mode == 'inference':

        batch_size = kwargs['batch_size']
        tdim      = config['tdim'] # int(500/dt) + fix_offs
        stim_mod  = 1 # rng.choice([1,2])

        if 'probe' in config and config['probe']=='probe':
            fix_offs  = tdim # stim_ons + int(rng.uniform(500,1500)/dt)
            stim_ons  = 0 # int(rng.uniform(300,700)/dt)
            stim_offs = tdim
        elif 'probe' in config and config['probe']=='fixed':
            stim_ons = int(tdim * 1 / 4)
            stim_offs = int(tdim * 2 / 4)
            fix_offs =  int(tdim * 3 / 4)
        else:
        ...
        # see if need to fix the yloc saccade location
        if 'y_loc' in config and isinstance(config['y_loc'], np.ndarray) and len(config['y_loc'])==batch_size:
            if anti_response: stim_locs = (config['y_loc']+np.pi)%(2*np.pi)
            else:  stim_locs = config['y_loc']
        else: stim_locs = rng.rand(batch_size)*2*np.pi
    
    elif mode == 'random': # Randomly generate parameters
        ...
        stim_ons  = int(rng.choice([300, 500, 700])/dt) # int(500/dt)
        stim_offs = stim_ons + int(rng.choice([200, 400, 600])/dt) # stim_ons + int(200/dt)
        fix_offs = stim_offs + int(rng.choice([200, 400, 800, 1600])/dt) # stim_offs + int(rng.choice([1600])/dt)
        tdim     = fix_offs + int(500/dt)

        if 'mixed_batch' in config and config['mixed_batch']: 
            scale = config['tdim'] / tdim
            stim_ons  = int(stim_ons  * scale)
            stim_offs = int(stim_offs * scale)
            fix_offs  = int(fix_offs  * scale)
            tdim = config['tdim']
    ...
    trial = Trial(config, tdim, batch_size)
    trial.add('fix_in', offs=fix_offs)
    if mode == 'inference' and 'no_stim' in config and config['no_stim']==1:
        pass
    else:        
...
def reactoffgo_(config, mode, anti_response, **kwargs):
    '''
    Fixate when fixation point is shown,
    A stimulus will be shown, and the output should saccade to the stimulus location
    Generate one batch of trials
    ...
    if mode == 'inference':
        ...
        if 'probe' in config and config['probe']=='probe':
            ...
            stim_ons  = int(tdim * 1 / 4)
            stim_offs  = int(tdim * 3 / 4)

        else: # essentially random trial
            stim_ons = int(rng.uniform(0,250)/dt)
            stim_offs = int(rng.uniform(1000,2000)/dt)
            tdim = int(500/dt) + stim_offs

            # max_norm = 2500 / dt # set a maximum value for the stim
            # stim_offs = int(rng.uniform(500,2000)/dt) # stim_ons + int(rng.uniform(500,1500)/dt)
            # stim_offs = int(stim_offs / max_norm * tdim)
        ...
        # A list of locations of stimuluss (they are always on)
        stim_locs = rng.rand(batch_size)*2*np.pi # rng.uniform(0, 2*np.pi, (batch_size,))
        stim_mod  = rng.choice([1,2])
    ...
    trial.epochs = {'fix1'     : (None, stim_offs),
                   'go1'      : (stim_offs, None)}

    return trial


def reactoffgo(config, mode, **kwargs):
    return reactoffgo_(config, mode, False, **kwargs)


def reactoffanti(config, mode, **kwargs):
    return reactoffgo_(config, mode, True, **kwargs)
...
rule_mapping = {'testinit': test_init,
                'fdgo': fdgo,
                'reactgo': reactgo,
                'reactoffgo': reactoffgo,
                'delaygo': delaygo,
                'fdanti': fdanti,
                'reactanti': reactanti,
                'reactoffanti': reactoffanti,
                'delayanti': delayanti,
                'dm1': dm1,
                'dm2': dm2,
...
def generate_trials(rule, hp, mode, noise_on=True, **kwargs):
    ...
    trials = []
    for i,task in enumerate(rule):

        trial = rule_mapping[task](config, mode, **kwargs)
        ...
        for r, s in zip(task, rule_strength):
    ...
    if len(trials)==1: return trials[0]
    if 'mixed_batch' not in config or not config['mixed_batch']: return trials


    ### time to combine all the trials, rule = list of tasks
    # 'float_type', 'config', 'dt', 'n_eachring', 'n_input', 'n_output', 'pref', 'batch_size', 'tdim', '_sigma_x', 
    #  'x', 'y', 'y_loc', 'c_mask', 'epochs'
    main_trial = trials[0] # Trial(trials[0].config, trials[0].tdim, len(rule))
        # c_mask = trials[0].c_mask
    # main_trial.float_type = trials[0].float_type; main_trial.dt = trials[0].dt
    # main_trial.n_input = trials[0].n_input; main_trial.n_output = trials[0].n_output
    # main_trial.pref = trials[0].pref; main_trial._sigma_x = trials[0]._sigma_x
    # main_trial.n_eachring = trials[0].n_eachring

    for i,t in enumerate(trials):
        if i==0: continue
        main_trial.x = np.hstack((main_trial.x, t.x))
        main_trial.y = np.hstack((main_trial.y, t.y))
        main_trial.y_loc = np.hstack((main_trial.y_loc, t.y_loc))
        main_trial.epochs = np.vstack((main_trial.epochs, t.epochs))
        main_trial.c_mask = np.vstack((main_trial.c_mask, t.c_mask))
